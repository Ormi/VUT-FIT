...
int i;
	...
	/**
	 * Test na q = NULL
	 * Cyklus pre naplnenie zoznamu *
	 * Priradenie prveho prvku a prvej volnej pozici 0
	 */
	if(q == NULL)
	{
		queueError(QERR_INIT);
...
*/
	...
	/**
	 * Test na prazdnost fronty cez implemenetovanu funkciu
	 * Inak vrat hodnotu znak zo zaciatku fronty cez index fronty
	 */
	if(queueEmpty(q))
	{
...
*/
	...
	/**
	 * Test na prazdnost fronty cez implemenetovanu funkciu
	 * Inak aktualnehmu indexu znaku prirad nasledujuci index
	 */
	if(queueEmpty(q))
	{
...
*/
	...
	/**
	 * Nejde vkladat znak do plnej fronty
	 * Inak posun ukazatel a pridaj znak
	 */
	if(queueFull(q))
	{
		queueError(QERR_UP);
